...
**Decision.** `internal/session` owns the file at the contractual `~/.agentrepl/logs/<session-id>.jsonl`. It is deliberately tiny — agentkit does the writing (its `LogRecord` protocol via `Conversation.Log`); session mints the id, names the default directory, and opens the file.
...
// DefaultDir returns the contractual default log directory under home:
// <home>/.agentrepl/logs.
...
- The composition root resolves `dir` as `session.DefaultDir(os.UserHomeDir())` — `~/.agentrepl/logs`, beside the default auth file, so agentrepl's entire on-disk footprint sits under `~/.agentrepl/` — and passes it in; tests pass a temp dir. `Open` does `os.MkdirAll(dir, 0o755)` (creating nested path segments as needed), then opens `O_CREATE|O_WRONLY|O_TRUNC`, mode `0o644`, **unbuffered** (Decision 6's no-torn-line guarantee). The prior `~/.agentkit` location is abandoned, not migrated: old logs are inert forensic files, and nothing reads them back.
...
- **Keeping `~/.agentkit` as the log home** — the directory name suggested agentkit owns the logs, but the path was always agentrepl's invention (agentkit takes only an `io.Writer`); one app, one dotdir.
- **Migrating existing `~/.agentkit/*.jsonl` files** — write-only forensic output with no reader; moving them buys nothing and risks clobbering.

**Verification.**
- R-8GF4-LRYU — `ID(t)` is deterministic for a given `t`, and `Open` targets `<dir>/<id>.jsonl`.
- R-GUN2-2ULO — `DefaultDir(home)` returns exactly `<home>/.agentrepl/logs` (not `<home>/.agentkit`), and `Open` over a `DefaultDir`-shaped nested path creates both missing segments and writes there.
