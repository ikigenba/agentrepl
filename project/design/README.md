...
- **Dependency:** built on `github.com/ikigenba/agentkit`; agentrepl drives `*agentkit.Conversation` directly. It builds against the **restructured** agentkit surface (credential constructors, the advisory catalog, consumer-owned cost resolution); a version predating that restructure (legacy model constants, `ReasoningInspector`, library-internal pricing) cannot satisfy this design. agentrepl consumes — and reimplements none of — this agentkit surface:
  - **Credential constructors:** per-package sealed `Credential` types — `anthropic.APIKey`, `google.APIKey`, `openai.APIKey`, `openai.Subscription(TokenSource)`, `openrouter.APIKey`, `zai.APIKey` — with `pkg.New(cred, opts...)` and the universal option `WithBaseURL(string)`; plus `openai/subscription`'s `Load(path)`, `(*Store).Token`, `Login(ctx, path, LoginIO)`. The library reads no env vars; agentrepl supplies key material (Decisions 2/14).
  - **Advisory catalog:** `agentkit/catalog` — `Entry{Model, Provider, Routes, Pricing, Reasoning, Context, Embedding, …}`, `Lookup`, `Resolve`, `ListByProvider`, `Check`. Advisory by contract: catalog coverage never controls what may be sent (free-flow); agentrepl's gating policy is its own (Decision 3).
  - **Native value carrier:** `agentkit.ReasoningValue` with constructors `Level(string)`, `Budget(int)`, `DisableReasoning()`; the zero value = unset → provider default. It is **opaque** (no read-back), which is why config keeps a separate display string (Decision 3). `agentkit.ReasoningSpec{Term, Kind, Levels, Min, Max, Sentinels, Default, CanDisable}` (+ `ReasoningKind`, `Sentinel`) now reaches agentrepl only through catalog entries — the per-package inspectors are gone.
  - **Cost resolution (consumer-owned):** per-turn cost resolves provider-reported cost (OpenRouter) → `Conversation.Pricing` (agentrepl installs the cataloged `Entry.Pricing`, Decision 3) → `0` + `WarnCostUnknown` (Decision 7).
  - **Standard toolkit:** `agentkit/toolkit` (agentkit ≥ v0.7.0) — `All(root string) []agentkit.Tool`, the six standard coding tools `Bash`, `Read`, `Write`, `Edit`, `Glob`, `Grep`, file access confined beneath `root`. agentrepl wires `All` with the process cwd and ships no tools of its own (Decision 10).
  - **Warning surface:** `agentkit.Warning{Setting, Code, Detail}`, `agentkit.WarningCode` (`WarnToolChoiceForced`, `WarnToolSchemaLossy`, `WarnCostUnknown`), read after a turn via `(*agentkit.Stream).Warnings() []Warning`. The former reasoning warning codes were removed with the inspectors — reasoning values pass through to the provider, and gating cataloged models is agentrepl's job (Decision 3).
