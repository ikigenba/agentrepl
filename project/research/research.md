...
- **The chat/embedding-model roster is agentkit's to own and evolve; this doc does not duplicate exact ids or counts** (that data lives solely in agentkit's `catalog` package and would go stale here on every catalog bump). What holds structurally, independent of the current roster: `ListByProvider` can return provider-native entries (no cross-provider `Routes` entry) alongside routed entries under the same provider — e.g. a model whose home is `zai` also appears under `openrouter` via a `Routes["openrouter"]` wire slug, while an OpenRouter-native model appears under `openrouter` with no route at all. `Embedding != nil` marks an embedding entry. Each populated `Reasoning` spec carries the terms/levels/ranges/sentinels/defaults its `Kind` (enum/range/toggle) calls for.
