...
- **Launch into a conversation, immediately.** Running agentrepl with a provider and model selected drops the operator into an interactive prompt where they type a message and get a reply. Initial settings come from flags; nothing else is required to start talking.
...
- Launching agentrepl with a provider and model selected yields an interactive prompt; typing a message returns a coherent reply.
