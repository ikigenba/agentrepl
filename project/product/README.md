...
- **Usable defaults.** Launching with no flags starts a working selection — OpenAI's `gpt-5.6-sol` under subscription auth — so the shortest path from install to conversation is zero configuration (plus a one-time login).
...
- **Two auth methods for OpenAI** — an API key, or a ChatGPT-subscription login held in a local auth file that agentrepl can create itself through an in-REPL login command. Every other provider authenticates with its API key. API keys come only from the environment, one conventional variable per provider.
...
- **No credential handling beyond the environment and the subscription auth file.** No keys on the command line, no credential prompts or stores beyond the login flow that writes the auth file; agentrepl never displays or edits the file's contents.
...
- **Subscription auth file:** defaults to `~/.agentrepl/auth.json`; its location is a config setting (`auth_file`), so an existing Codex CLI login can be pointed at instead.
...
- **Missing credentials direct, they don't crash.** Starting with no keys and no login always reaches the prompt; the first turn that needs the missing credential explains exactly what to do — log in with the runtime command, switch to an API key, or point at an existing login file — and the session stays usable throughout.

- **Log in from inside the REPL.** A runtime login command walks the operator through the subscription sign-in (open a URL, paste the code back) and stores the result in the auth file; the very next turn uses it, no restart. Subscription-backed usage shows its cost figures like any other usage; those figures are notional rather than an additional charge.
...
- Launching with no credentials at all still reaches the prompt; the first turn explains how to log in, switch to an API key, or point at an existing login file — and the session remains usable.
- The in-REPL login command completes the subscription sign-in and the next turn works without a restart.
