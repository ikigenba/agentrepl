...
Phases 1–10 done (the full REPL plus the configurable Z.ai base URL), built against agentkit `v0.1.0`'s universal `ReasoningEffort` enum. Phases 11–14 done for the **native-first reasoning** consumer change (design D2/D3/D5/D12 rewritten in place, plus the breaking agentkit pin bump). Phase 15 done for the `--help` reasoning-row rework (design Decision 12 amended so each row leads with the literal `gen.reasoning=` key). Phases 16–17 done for the **flatten-to-passthrough** change (design D3/D12 amended in place: the `gen.`/`retry.`/`zai.` prefixes dropped and the single `gen.reasoning` key replaced by four native keys — `effort`/`thinking_budget`/`thinking_level`/`thinking`). Phase 18 is appended for the **decorated input prompt & per-turn-report removal** change (design D5/D7/D9 amended in place: the `Prompt()`/`Input(text)` interface split, the `tty`-gated `you ›` prompt with no input echo, the removed `─` separator, and per-turn spend made raw-only): ✅ done. Phases 19–21 are appended for the **decorated UX cleanup** pass (design D5 amended in place with the color palette and the leading-separator vertical spacing, and new Decision 13 — the `waiting for <model>` status line — added, with its seam recorded in D1 and its driver wiring in D5): ⬜ not started.
...
### Phase 18 — Decorated input prompt & per-turn-report removal · ✅ done
...
### Phase 19 — Decorated palette & vertical spacing · ✅ done
