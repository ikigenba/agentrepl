...
Phases 1–10 done (the full REPL plus the configurable Z.ai base URL), built against agentkit `v0.1.0`'s universal `ReasoningEffort` enum. Phases 11–14 are appended for the **native-first reasoning** consumer change (design D2/D3/D5/D12 rewritten in place, plus the breaking agentkit pin bump): ⬜ not started.
